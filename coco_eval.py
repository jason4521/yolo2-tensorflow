    ...
    for i in range(grid_h):
        for j in range(grid_w):
            for k in range(n_anchors):
                ...
                class_prob = objectness * class_vec  # 该限位框属于各个类别的概率

                # 得到图像缩放之后尺度下的绝对坐标下限位框的左上角坐标和宽度与高度
...
'''
    ...
    # 执行阈值操作，即当某一个限位框属于某一类别的概率小于roi_prob时
    # ，则将此概率设置为零，以便忽略此属于关系
        ...
        for loc_idx, class_prob in enumerate(class_):
            if class_prob>0:
                ...
                cv2.putText(img, text, (x, y-8), cv2.FONT_HERSHEY_SIMPLEX, 
                              0.7, colors[class_idx], 2, cv2.LINE_AA)
    return img
...
net_scale = 32.0
grid_w, grid_h = 39, 22  # 分别表示在宽度和高度尺度上的网格单元个数
